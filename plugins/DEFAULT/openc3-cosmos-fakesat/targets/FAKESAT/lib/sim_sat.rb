...
module OpenC3
  ...
  class SimSat < SimulatedTarget
    def initialize(target_name)
      ...
      @test_temp = 0.0 # NEW: Initialize test temperature
    ...
    def write(packet)
      ...
      when 'SET_TEST_TEMP' # NEW: Handle SET_TEST_TEMP command
        new_temp = packet.read('NEW_TEMP')
        @test_temp = new_temp
        accept_cmd("Set Test Temperature to #{@test_temp}")
      end
    end
    ...
    def read(count_100hz, time)
      ...
      pending_packets.each do |packet|
        ...
        when 'HEALTH_STATUS'
          ...
          @test_temp += (rand(2) - 1) * 0.1 # NEW: Small fluctuation for test temp
          packet.test_temp = @test_temp # NEW: Update test temperature telemetry
